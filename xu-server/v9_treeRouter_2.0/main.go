...
func main() {
	server := NewHttpServer("Server_test", "tree", MetricsFilterBuilder1, MetricsFilterBuilder2)
	//server.Route("/", home)
	//server.Route("/user", user)
	//server.Route("/user/create", createUser)
	//server.Route("/order", order)
	//server.Route("/user/signUp", SignUp)
	server.Route(http.MethodPost, "/test/*", Test)
	server.Route(http.MethodPost, "/user/*", User)
