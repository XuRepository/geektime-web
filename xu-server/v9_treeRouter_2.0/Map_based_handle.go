...
func (h *HandlerBasedOnMap) Route(method string, pattern string, handlerFunc handlerFunc) error {
...
func NewHandlerBasedOnMap() *HandlerBasedOnMap {
	println("new map router")
	return &HandlerBasedOnMap{handlers: make(map[string]func(ctx *Context))}
}

// ServerHTTP 实现Handler接口，这样就可以把HandlerBasedMap作为服务器的Handler，处理注册的路由！
//实现的功能是  处理http请求，判断请求的类型和路径，创建context封装resp和req，从map中取到相应的handlerFunc进行处理！
func (h *HandlerBasedOnMap) ServerHTTP(ctx *Context) {
